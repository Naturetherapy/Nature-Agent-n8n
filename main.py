    ...
    try:
        ...
        for track in results:
            ...
            if t_id not in history:
                ...
                r = requests.get(m_url, timeout=20)
...
def run_automation():
    ...
    v_resp = requests.get(f"https://api.pexels.com/videos/search?query={topic}&per_page=15&orientation=portrait", 
                          headers={"Authorization": PEXELS_API_KEY}, timeout=15).json()
            ...
            if os.path.exists("final.mp4"):
                try:
                    with open("final.mp4", 'rb') as f:
                        # MODIFIED: Timeout increased to 120s to prevent ReadTimeout error
                    ...
                    if merged_url.startswith('http'):
                        ...
                        requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendVideo", 
                                      data={"chat_id": TELEGRAM_CHAT_ID, "caption": full_post_text}, 
                                      files={"video": open("final.mp4", 'rb')}, timeout=60)
                        
                        # Webhook Redirect to Make.com (Using increased timeout)
                        if MAKE_WEBHOOK_URL:
                    ...
                    print(f"Error during upload/webhook: {e}")
                    continue
